...
function solve() {
  ...
  const clearInputFields = (dataFromInput) =>
    dataFromInput.forEach((x) => (x.value = ""));
  ...
  const editFunctionality = async (event) => {
    ...
    Object.values(inputFields).forEach(
      (x, index) => (x.value = data[index].textContent)
    );
  ...
  const loadApiToHtml = (data) => {
    ...
    Object.values(data).forEach((x) =>
      courseApp.list.appendChild(createHtmlElement(x))
    );
  };
