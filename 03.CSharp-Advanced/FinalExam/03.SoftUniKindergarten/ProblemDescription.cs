//You are all familiar with the problem of finding an available place in a nursery school for children. 
//SoftUni has decided to help its employees and to build for their children a brand new, out-of - this - world, smartest possible building, 
//fully autonomous and energy independent, and last but not least, forever cost-free SoftUni Kindergarten. 
//You will participate in implementing some logic in the operating software.

//1. Preparation

//Download the skeleton provided in Judge. Do not change the StartUp class or its namespace.

//Pay attention to name the package kindergarten, all the classes, their fields, and methods the same way they are presented in the following document.
...
//Kindergarten
//Next, you are given a class Kindergarten. The Kindergarten will have Name and Capacity(the maximum number of children that can be registered), 
...
//• Method RemoveChild(string childFullName) - removes a Child by a given full name. 
//The childFullName will be a composition of the first name and the last name of the Child, separated by a single space.If removal is successful, returns True,
//otherwise, returns False.
//• Getter ChildrenCount - Returns the number of all children registered
//• Method GetChild(string childFullName) – Returns the Child with the given full name. . 
//The childFullName will be a composition of the first name and the last name of the child, 
//separated by a single space.If no child with the given childFullName is found, return null.
//• Method RegistryReport() – Orders the children by age descending, then by last name ascending, then by first name ascending, 
