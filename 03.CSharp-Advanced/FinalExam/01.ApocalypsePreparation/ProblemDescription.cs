...
//Until one of the collections contains no elements, you will have to combine elements from the collections in order to create healing items. 
//You should start by getting the first value of textile and the last value of medicaments and if their sum is equal to any of the items in the table below
//create that item and remove both values. Otherwise, check if the sum is bigger than the value of the MedKit, create the MedKit, remove both values,
//and add the remaining resources(of the sum) to the next value in the medicament collection (Take the element from the collection, add the remaining sum to it,
//and put the element back to its place). If you can’t create anything, remove the textile value, add 10 to the medicament value, 
//and return the medicament back to its place, into its collection. You need to stop creating healing items when either the textile or the medicaments are exhausted.
